...
const TeacherDashboard = () => {
  ...
  const [studentList, setStudentList] = useState([]); // Real data from DB
  ...
  // Prevent empty or very short names
  if (!newStudentName || newStudentName.trim().length < 2) {
  ...
  try {
    const response = await fetch('http://localhost:8080/api/users/add', {
      ...
      body: JSON.stringify({ 
        name: newStudentName.trim() // Sends the full name with spaces
      })
    });

    if (response.ok) {
      alert(`Successfully enrolled: ${newStudentName}`);
      setNewStudentName(""); // Clear the input box
      fetchStudents(); // This will refresh the table automatically
    } else {
      // This will show us the actual error from Java in the console
      const errorData = await response.json();
      console.error("Server Error:", errorData);
      alert("Enrollment failed: Check console for DB errors.");
    }
  } catch (error) {
  ...
  const DashboardHome = () => (
    <>
      ...
      <div className="grid grid-cols-3 gap-8 mb-10">
        <div className="col-span-2 bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 text-left">
    ...
    <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden text-left p-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-2">
          <input 
            type="text" placeholder="Enroll Student..." value={newStudentName}
            onChange={(e) => setNewStudentName(e.target.value)}
            className="px-4 py-2 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-100"
          />
