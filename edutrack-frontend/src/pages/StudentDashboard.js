...
const StudentDashboard = () => {
  ...
  // NEW: State for real-time date and clock
  ...
  useEffect(() => {
    ...
    if (storedId) {
      fetch(`http://localhost:8080/api/grades/${storedId}`)
        .then(res => res.json())
        .then(data => {
          if (data && data.length > 0) {
            // Take the most recent grade entry (last item in array)
            const marks = data[data.length - 1]; 
            setCourses(prev => [
      ...
      <aside className="w-64 bg-[#2e268a] text-white hidden xl:flex flex-col fixed h-full z-10">
        ...
        <nav className="flex-1 py-6 space-y-1">
          {['Dashboard', 'My Courses', 'Grades', 'Attendance', 'Calendar', 'Alerts', 'Profile'].map((item) => (
            <div key={item} className={`px-8 py-3 text-sm font-medium cursor-pointer flex items-center gap-4 border-l-4 ${item === 'Dashboard' ? 'bg-white/10 border-white text-white' : 'border-transparent text-indigo-200 hover:bg-white/5'}`}>
